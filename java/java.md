...
## 垃圾回收
不同的的对象的生命周期是不一样的。因此，不同生命周期的对象可以采用不同的回收算法。
### 年轻代（Eden）
所有新生的对象首先是放在Eden区。年轻代的目标就是尽可能快速回收生命周期短的对象。

对应Minor GC，每次Minor GC会清理年轻代的内存，算法采用效率较高的复制算法，但是会浪费内存空间。当“年轻代”区域存放满对象后，就将对象存放到老年代区域。
### 年老代（old）
在年轻代中经历GN（默认15）次垃圾回收后仍然存活的对象，就会被放到年老代中。因此，可以认为年老代中存在的都是一些生命周期较长的对象。年老代对象越来越多，我们就需要启动Major GC和Full GC（全量回收），全面清理年轻代和年老代。
### JVM调优和Full GC
在对JVM调优的过程中，很大一部分工作是对于Full GC的调节，有如下原因可能导致Full GC：
+ 年老代（Tenured）被写满
+ 永久代（Perm）被写满
+ System.gc()被显式调用
+ 上一次GC之后Heap的域分配策略动态变化
## 类
