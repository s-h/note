...
###  16.6. <a name='-1'></a>网格填充方式
**grid-auto-flow** 决定网格容器中元素**排列填充**方式
+ grid-auto-flow:row 默认，子元素按先后列顺序填充，优先填满一行后换行
+ grid-auto-flow:column 子元素按**先列后行**顺序填充，优先填满一列后换行

###  16.7. <a name='object-fit'></a>object-fit
**object-fit** 是CSS中用于控制替换元素（如img、video、iframe等加载外部资源的元素）内容如何适应其容器尺寸的属性。
| 值      | 描述                                                                             |
| ------- | -------------------------------------------------------------------------------- |
| contain | 保持内容宽高比，缩放内容                                                         |
| cover   | 保持宽高比，缩放并裁剪                                                           |
| fill    | 默认值。拉伸内容以完全填充容器，不保持宽高比，可能导致内容变形（最常用但需谨慎） |

###  16.8. <a name='-1'></a>项目对齐方式
在CSS Grid布局中，控制元素在网格内的对齐方式
+ justify-items： 水平对齐方式
+ align-items： 垂直对方式
+ place-items： 水平+垂直

justify-items、align-items 属性值
...
| 属性         | 作用                             | 示例值                      |
| ------------ | -------------------------------- | --------------------------- |
| column-count | 根据容器宽度和该值，自动分配列宽 | column-count:3;             |
| column-gap   | 列之间的间距                     | 20px, normal(默认 1em)      |
| column-rule  | 列之间分割线样式                 | column-rule: 1px solid #ccc |
